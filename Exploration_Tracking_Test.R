...
plot_elbow_method <- function(data, max_k) {
  ...
  ggplot(elbow_data, aes(x = K, y = WCSS)) +
    geom_line(color = "#377eb8", size = 0.5) +  # Blue line with increased size
    geom_point(color = "#377eb8", size = 1.5) +  # Blue points with increased size
    labs(
      title = "Elbow Method for Optimal K",
      x = "Number of clusters (K)", 
      y = "Within-Cluster Sum of Squares (WCSS)"
    ) +
    theme_minimal(base_size = 14) +  # Minimal theme with base font size set for better readability
    theme(
      plot.title = element_text(hjust = 0.5, face = "bold"),
      plot.subtitle = element_text(hjust = 0.5),
      text = element_text(family = "Times New Roman"),
      axis.title.x = element_text(margin = margin(t = 10)),  # Adjusting margin for x-axis title
      axis.title.y = element_text(margin = margin(r = 10)),  # Adjusting margin for y-axis title
...
#Initialize Exploration Counter Storage
cumulative_exploration_pooled <- data.frame(Instances = 1:nRecommendations)
cumulative_exploration_unpooled <- data.frame(Instances = 1:nRecommendations)
cumulative_exploration_partially <- data.frame(Instances = 1:nRecommendations)
###############################################################################
...
for (i in 1:nSimulation_Runs){
  ...
  results_pooled<-matrix(0,nrow=nRecommendations,ncol=5)# columns: 1=index, 2= arm, 3= result, 4= CTR, 5= Exploration tracker
  ...
  for (t in 1:nRecommendations){
    ...
    #If the selected arm does not have the highest alpha to bea ratio --> we count as exploration
    if((mAB_pooled[arm_pooled,1]/mAB_pooled[arm_pooled,2]) != max(apply(mAB_pooled, 1, function(row) row[1] / row[2]))){
      exploration <- 1} else {exploration <- 0}
    
    # Determine the outcome
    outcome_p <- rbinom(1, 1, Cluster_CTR[segment, arm_pooled+6])
    
    results_pooled[t,]<-c(t,arm_pooled,outcome_p,Cluster_CTR[segment, arm_pooled+6],exploration)
  ...
  cumulative_exploration_pooled[, paste0("Run_", i)] <- cumsum(results_pooled[,5])
  }
...
for (i in 1:nSimulation_Runs){
  ...
  ##Initialize Beta Distribution Table
  ...
  results_unpooled <- matrix(0, nrow = nRecommendations, ncol = 6) #added column 6 for exploration
  ...
  for (t in 1:nRecommendations){
    ...
    #If the selected arm does not have the highest alpha to bea ratio --> we count as exploration
    if((mAB_unpooled[arm_unpooled,1,segment]/mAB_unpooled[arm_unpooled,2,segment]) != max(apply(mAB_unpooled[,,segment], 1, function(row) row[1] / row[2]))){
      exploration <- 1} else {exploration <- 0}
    
    # Determine the outcome based on the arm's CTR
    outcome_un <- rbinom(1, 1, Cluster_CTR[segment, arm_unpooled+6])
    
    results_unpooled[t, ] <- c(t, arm_unpooled, outcome_un, segment,Cluster_CTR[segment, arm_unpooled+6],exploration)
  ...
  cumulative_exploration_unpooled[, paste0("Run_", i)] <- cumsum(results_unpooled[,6])
} 
...
for (i in 1:nSimulation_Runs){
  ...
  results_partially <- matrix(0, nrow = nRecommendations, ncol = 6)#added column 6 for exploration
  ...
  for (t in 1:nRecommendations){
    ...
    #If the selected arm does not have the highest alpha to bea ratio --> we count as exploration
    if((mAB_aggregated[arm_partially,1]/mAB_aggregated[arm_partially,2]) != max(apply(mAB_aggregated, 1, function(row) row[1] / row[2]))){
      exploration <- 1} else {exploration <- 0}
    
    # Determine the outcome based on the arm's CTR
    outcome_pa <- rbinom(1, 1, Cluster_CTR[segment, arm_partially+6])

    results_partially[t, ] <- c(t, arm_partially, outcome_pa, segment, Cluster_CTR[segment, arm_partially + 6],exploration)
    #UPDATE PROCESS:
  ...
  cumulative_exploration_partially[, paste0("Run_", i)] <- cumsum(results_partially[,6])
}
...
Exploration_plot <- ggplot() +
  geom_line(data = cumulative_exploration_pooled, aes(x = Instances, y = rowMeans(select(cumulative_exploration_pooled, -Instances)), color = "Pooled"), size = 0.5) +
  geom_line(data = cumulative_exploration_unpooled, aes(x = Instances, y = rowMeans(select(cumulative_exploration_unpooled, -Instances)), color = "Unpooled"), size = 0.5) +
  geom_line(data = cumulative_exploration_partially, aes(x = Instances, y = rowMeans(select(cumulative_exploration_partially, -Instances)), color = "Partially"), size = 0.5) +
  labs(
    title = "Average Cumulative Exploration over 10 Simulation Runs",
    x = "Instances (in thousands)", y = "Average Cumulative Exploration"
  ) +
  scale_color_manual(values = c("Pooled" = "#4daf4a", "Unpooled" = "#e41a1c", "Partially" = "#377eb8")) +
  scale_x_continuous(labels = function(x) paste0(x/1000, "")) +  # Formatting x labels in thousands
...
avg_total_exploration_pooled <- sum(cumulative_exploration_pooled[nRecommendations,2:11])/nSimulation_Runs
avg_relative_exploration_pooled <- sum(cumulative_exploration_pooled[nRecommendations,2:11])/nRecommendations*nSimulation_Runs

avg_total_exploration_unpooled <- sum(cumulative_exploration_unpooled[nRecommendations,2:11])/nSimulation_Runs
avg_relative_exploration_unpooled <- sum(cumulative_exploration_unpooled[nRecommendations,2:11])/nRecommendations*nSimulation_Runs

avg_total_exploration_partially <- sum(cumulative_exploration_partially[nRecommendations,2:11])/nSimulation_Runs
avg_relative_exploration_partially <- sum(cumulative_exploration_partially[nRecommendations,2:11])/nRecommendations*nSimulation_Runs
